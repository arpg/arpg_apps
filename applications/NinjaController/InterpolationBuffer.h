...
struct InterpolationBufferT {
  ...
  void AddElement(const ElementType& element) {
    assert(element.timestamp > end_time);
    const size_t nElems = elements.size();
    const ScalarType dt = nElems == 0 ? 0 : element.timestamp - elements.back().timestamp;
    ...
    end_time = element.timestamp;
    start_time = elements.front().timestamp;
  }
  ...
  bool GetNext(const ScalarType max_time, size_t& index_out,
    ...
    if (index_out + 1 >= elements.size()) {
      ...
      return false;
    } else if (elements[index_out + 1].timestamp > max_time) {
      output = GetElement(max_time, &index_out);
      return false;
    } else {
      index_out++;
  ...
  ElementType GetElement(const ScalarType time, size_t* pIndex) const {
    ...
    if (elements[guess_idx].timestamp > time) {
      ...
      while ((guess_idx - 1) > 0 && elements[guess_idx - 1].timestamp > time) {
        guess_idx--;
      }
      const ScalarType interpolator = (time - elements[guess_idx - 1].timestamp)
          / (elements[guess_idx].timestamp - elements[guess_idx - 1].timestamp);
      ...
      *pIndex = guess_idx - 1;
      ...
      while ((guess_idx + 1) < n_elements &&
             elements[guess_idx + 1].timestamp < time) {
        guess_idx++;
      }
      const ScalarType interpolator = (time - elements[guess_idx].timestamp)
          / (elements[guess_idx + 1].timestamp - elements[guess_idx].timestamp);
